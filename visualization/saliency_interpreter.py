...
class SaliencyInterpreter:
    ...
    def reconstruct_text(self, token):
        # Mapping special tokens to their text equivalents
        special_mappings = {
            "<s>": "",        # Start token - ignore
            "</s>": "",       # End token - ignore
            "<0x0A>": "<br>",   # Newline
            "<0x09>": "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"    # Tab
            # "<0x0A>": "\n",   # Newline
        ...
        if token in special_mappings:
            return special_mappings[token]
    ...
    def colorize(self, instance, skip_special_tokens=False):
        ...
        for word, color in zip(instance['tokens'], normalized_and_mapped):
            ...
            word = self.reconstruct_text(word)
            color = matplotlib.colors.rgb2hex(color[:3])
            if word == "<br>":
                colored_string += word
            else:
                colored_string += template.format(color, word)
        # colored_string += template.format(0, "<br>    Label: {} |".format(instance['label']))
        # prob = instance['prob']
    ...
    def update_output(self):
        ...
        probs, labels = probs.squeeze(0), labels.squeeze(0)
        ...
        norms = torch.norm(embedding_grads, dim=1, p=2)
        # normalizing
